...
const Curriculum = () => {
  ...
  return (
    <SectionContainer bgColor="#ffffff">
      <div>
        <h2 className="text-center text-[24px] lg:text-[30px] font-bold mb-6 lg:mb-[35px]" style={{ color: "#232833" }}>
          Course <span style={{ color: "#264f9b" }}>Curriculum</span>
        </h2>

        <div className="flex flex-col lg:flex-row gap-6 lg:gap-[30px] bg-[#fefefe] p-2 lg:p-[6px] rounded-[12px]">
          <div className="w-full lg:w-[45%] border border-[#c6c8cc] rounded-[8px] overflow-hidden self-start">
            {lecturesData.map((course, i) => (
              <div
                key={i}
                onClick={() => setActiveIndex(i)}
                className={`relative cursor-pointer transition px-3 py-2 lg:px-[16px] lg:py-[10px] border-b border-[#c6c8cc] ${activeIndex === i
              ...
              >
                ...
                <h4 className={`mt-[2px] text-[13px] lg:text-[13.5px] leading-[1.2] font-semibold ${activeIndex === i ? "text-[#264f9b]" : "text-[#4e5666]"}`}>
                  {course.name}
                </h4>
