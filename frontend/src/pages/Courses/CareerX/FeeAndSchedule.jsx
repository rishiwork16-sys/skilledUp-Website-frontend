...
const FeeAndSchedule = ({ courseDetails }) => {
  ...
  const handleEnrollNow = async () => {
    ...
    try {
      ...
      if (!user) {
        ...
        if (shouldLogin) {
          window.location.href = '/login?returnUrl=/careerx';
        }
      ...
      // 1. Create Order
      const orderData = await createPayment(courseDetails.id, JSON.parse(user));
      ...
      if (orderData.amount <= 0) {
          ...
          // Direct verification for free course (you might need a specific endpoint for this, 
          // or just assume success if your backend handles it on create-order)
          // For now, we'll try to verify with dummy data or just redirect
          try {
             // If backend requires verification even for free courses, send dummy successful response
             // OR: If your createPayment already granted access, just redirect.
             // Assuming we need to 'close' the loop:
      ...
      const options = {
        ...
        handler: async (response) => {
          ...
          try {
            const result = await handlePaymentSuccess(response, courseDetails.id);
      ...
      } else if (error.message.includes('Cannot deserialize value')) {
        errorMessage += 'Invalid course ID. Please contact support.';
      } else {
      ...
      <div id="course-fee-section">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8 lg:gap-6">
              ...
              {filteredSlots.map((slot) => (
                ...
                >
                  ...
                  <ul className="list-none p-0 m-0 text-[10px] lg:text-[16px] leading-[1.7] space-y-0">
                    ...
                    <li><b>Mode:</b> {slot.mode === 'online' ? 'Virtual' : slot.mode === 'offline' ? 'In-person' : 'Mixed'}</li>
                  </ul>
                </div>
              ))}
            </div>
            ...
            <div className="w-full lg:w-[35%] text-center lg:text-left lg:pl-4">
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-end gap-4 justify-center lg:justify-start">
                  <span className="text-[42px] lg:text-[48px] font-extrabold text-gray-900 leading-none">
                    ₹{courseDetails.price.toLocaleString("en-IN")}
                  </span>
                ...
                {/* {courseDetails.discount > 0 && (
                  <div className="inline-flex items-center gap-2 bg-blue-50 text-[#0057ff] text-[14px] lg:text-[15px] font-semibold px-4 py-1.5 rounded-full mx-auto lg:mx-0">
                    Flat {courseDetails.discount}% OFF - Ends Soon
                  </div>
                )} */}
              </div>
              ...
              <button
                ...
                className="mt-[22px] px-[26px] py-[11px] bg-[#264f9b] text-white text-[16px] font-semibold rounded-[10px] transition hover:bg-[#191917] w-full lg:w-auto disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isProcessingPayment ? (
        ...
        <div className="w-full mt-[16px] lg:mt-[20px]">
          <img
            ...
            className="w-full rounded-[15px] shadow-[0_4px_12px_rgba(0,0,0,0.12)]"
          />
        </div>
      </div>
    </SectionContainer>
